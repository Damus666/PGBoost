...
class UIEntryLine(UIElement):
    ...
    def draw(self, surface,offset=(0,0)):
        if self.absolute_rect.colliderect(self._container.absolute_rect):
            ...
            if self._is_focused and (self._cursor_on or Time.ticks-self._time_since_last <= self._time_gap):
    ...
    def update(self, dt, arg=None):
        if Time.ticks - self._last_cursor_update >= self._cursor_blink_time:
            self._last_cursor_update = Time.ticks
            self._cursor_on = not self._cursor_on
        if self._last_key_pressed != None:
            if Input.GetKey(self._last_key_pressed):
                if Time.ticks - self._t >= self._ct:
