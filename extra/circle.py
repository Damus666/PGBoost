...
class Circle():
    ...
    @staticmethod
    def FromRect(rect: pygame.Rect, color: Union[str, Tuple[int, int, int]] = "white", line_width: int = 0):
        """
    ...
    def Clip(self, circle):  # needs math optimisation
        ...
        """
        length = Math.Dist(self.center, circle.center)
    ...
    def Contains(self, circle) -> bool:
        ...
        """
        return Math.Dist(self.center, circle.center) + circle.radius <= self.radius

    def CollidePoint(self, x: int, y: int) -> bool:
        """
        Checks if a point is colliding this circle.
        """
        return Math.Dist(self.center, (x, y)) <= self.radius

    def CollideCircle(self, circle) -> bool:
        """
        Checks if two circles are colliding.
        """
        return Math.Dist(self.center, circle.center) < self.radius + circle.radius
    ...
    def CollideList(self, circles: list) -> tuple:
        ...
        for i in range(len(circles)):
            if Math.Dist(self.center, circles[i].center) < self.radius + circles[i].radius:
    ...
    def CollideDict(self, circles, use_values: bool = False) -> tuple:
        ...
        if not use_values:
            for circle in circles:
                if Math.Dist(self.center, circle.center) < self.radius + circle.radius:
                    return tuple((circle, circles[circle]))
        else:
            for key in circles.keys():
                if Math.Dist(self.center, circles[key].center) < self.radius + circle[key].radius:
    ...
    def CollideDictAll(self, circles: dict, use_values: bool = False) -> List[tuple]:
        ...
        if not use_values:
            ...
            for key in circles.keys():
                if Math.Dist(self.center, circles[key].center) < self.radius + circles[key].radius:
